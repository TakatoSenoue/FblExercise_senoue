...
{
static V_MEMRAM1 vuint16 V_MEMRAM2 applFblTaskTimer;
#if defined( FBL_ENABLE_VECTOR_HW )
# if defined( FBL_ENABLE_VECTOR_HW_SEND_MESSAGE )
   /* TODO_CUSTOMER: Remove the code regarding message sending, otherwise the TP connection could be influenced. */
#  if defined( kFblBrsApplTaskMsgBufferLength )
   vuint8 msgData[kFblBrsApplTaskMsgBufferLength] = {0u}; /* PRQA S 3678 */ /* MD_MSR_Rule8.13 */
#  endif /* kFblBrsApplTaskCanBuffer */

#  if defined( FBLWRAPPERCOM_CAN_VERSION )
   static tMsgObject myCanTxObj;
   vuintx idx = 0u;
#  else /* FBLWRAPPERCOM_CAN_VERSION */
#   if defined( FBLCW_CAN_BUSTYPEOFDCMDSLCONNECTIONINFO )
   Can_PduType canHandle;
#   endif /* FBLCW_CAN_BUSTYPEOFDCMDSLCONNECTIONINFO */
#  endif /* FBLWRAPPERCOM_CAN_VERSION */
# endif /* FBL_ENABLE_VECTOR_HW_SEND_MESSAGE */
#endif /* FBL_ENABLE_VECTOR_HW */

   applFblTaskTimer++;
   if (applFblTaskTimer >= 500u)
   {
      applFblTaskTimer = 0u;
#if defined( FBL_ENABLE_VECTOR_HW )
...
# if defined( FBL_ENABLE_VECTOR_HW_SEND_MESSAGE )
      /* Prepare the frame */
#  if defined( FBLWRAPPERCOM_CAN_VERSION )
      FblCanSetTxId(myCanTxObj, MK_STDID(kFblBrsApplTaskMsgBufferId));  /* ID of dummy message */
      FblCanSetTxDLC(myCanTxObj, kFblBrsApplTaskMsgBufferLength);       /* Set message length */
#   if defined( FBL_ENABLE_CAN_CONFIRMATION )
      FblCanSetTxConfirmation(myCanTxObj, V_NULL);
#   endif /* FBL_ENABLE_CAN_CONFIRMATION */
      for (idx = 0u; idx < kFblBrsApplTaskMsgBufferLength; idx++)
      {
         myCanTxObj.msgObject.DataFld[idx] = msgData[idx];              /* Set data for message */
      }
#  else /* FBLWRAPPERCOM_CAN_VERSION */
#   if defined( FBLCW_CAN_BUSTYPEOFDCMDSLCONNECTIONINFO )
      canHandle.id = (Can_IdType)kFblBrsApplTaskMsgBufferId;            /* ID of dummy message */
      canHandle.length = kFblBrsApplTaskMsgBufferLength;                /* Set message length */
      canHandle.sdu = &msgData[0u];                                     /* Set pointer to message data */
      canHandle.swPduHandle = 0u;
#   endif /* FBLCW_CAN_BUSTYPEOFDCMDSLCONNECTIONINFO */
#  endif /* FBLWRAPPERCOM_CAN_VERSION */

      /* Transmit Testmessage.
       * Use only for debug purposes. Don't use this method to send messages for production.
       */
#  if defined( FBLWRAPPERCOM_CAN_VERSION )
      while (FblCanTransmit(&myCanTxObj) != kFblCanTxOk)
#  else /* FBLWRAPPERCOM_CAN_VERSION */
#   if defined( FBLCW_CAN_BUSTYPEOFDCMDSLCONNECTIONINFO )
      while (Can_Write(0u, &canHandle) != CAN_OK)
#   endif /* FBLCW_CAN_BUSTYPEOFDCMDSLCONNECTIONINFO */
#  endif /* FBLWRAPPERCOM_CAN_VERSION */
#  if defined( kFblBrsApplTaskMsgBufferLength )
      {
...
#  endif /* kFblBrsApplTaskMsgBufferLength */
# endif /* FBL_ENABLE_VECTOR_HW_SEND_MESSAGE */
#endif /* FBL_ENABLE_VECTOR_HW */
   }
}
