...
void BrsMain_Appl_HypervisorStartCoreApi(uint32 coreID)
{
  ...
  /* If additional cores need to be started by the boot core, this needs to be done previously to the call of vHyp_Init(),
   * to prevent issues, if boot core is not the same as vHyp init core (VHYP_CORE_INIT).
